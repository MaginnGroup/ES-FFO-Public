...
def check_eq(job):
    ...
    n_vap = np.mean(eq_col_liq_molec[results["t0"]:])
    ...
    # Estimate the slope of the # of molecs of the liquid box vs step number
    custom_args, custom_args_gemc = _get_custom_args(job)
    steps = np.arange(0, len(eq_col_liq_molec)) * custom_args["prop_freq"]
    win_len = max(3, int(len(eq_col_liq_molec) * 0.1) | 1)
    dydx = savgol_filter(eq_col_liq_molec, window_length=win_len, polyorder=2, deriv=1)
    eq_col_est = savgol_filter(eq_col_liq_molec, window_length=win_len, polyorder=2)

    #Plot the number of molecules in the vapor box vs step number and the slope
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8))
    ax1.plot(steps, eq_col_liq_molec, label="Original Data", alpha=0.5)
    ...
    plt.tight_layout()
    plt.savefig(job.fn("liq_molec_slope.png"))
