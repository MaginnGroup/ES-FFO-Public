...
def create_system(job):
    ...
    # Calculate the xy box length given that z = aspect_ratio*box_xy
    xy_len = (V / job.sp.aspect_ratio) ** (1 / 3)
    # Calculate Z assuming that the box volume must be 1.15*expt_liq_dens and z>3*xy
    z_len = 1.15 * job.sp.aspect_ratio * xy_len
...
@Project.operation(with_job=True, cmd=False, directives={"omp_num_threads": 12})
def em_sim(job):
    import glob
    import shutil
...
@Project.operation(cmd=False, directives={"omp_num_threads": 12})
...
def inter_eq_sim(job):
    ...
    if not job.isfile("inter_eq/inter_eq.mdp"):
        with job:
            ...
            cutoff = np.minimum(
                round(0.95 * min_coord / 2, 5), round(6 * job.sp.max_sigma, 5)
            )
...
def calculate_props(job):
    ...
    for prop, name in zip(get_props, names):
        ...
        if prop == "Density":
            with job:
                (means_est, vars_est, vars_err) = calc_block_densities(
                    job, sim_name, last_sim_name, name
                )
...
def find_bulk_liq_index(density):
    ...
    if interfaces[0] < interfaces[1]:
        ...
        range_org_liq = list(range(interfaces[0], len(results) // 10)) + list(
            range(0, interfaces[1] + 1)
        )
...
def run_md_wo_eqcheck(job, sim_name, last_sim_name):
    try:
        with job:
            ...
            if sim_name != "em":
                w_gpu = " -ntomp 12 -nb gpu -pme gpu -bonded gpu -pin on"
    ...
    with job:
        ...
        try:
            ...
            if max_steps_str not in job.doc:
                job.doc[max_steps_str] = int(nsteps_eq / 1000) * 8
            ...
            max_eq_steps = np.maximum(
                job.doc[max_steps_str], total_eq_steps + eq_extend * 40
            )
                ...
                if total_eq_steps + eq_extend <= max_eq_steps:
                    ...
                    if total_eq_steps == 0:
                        command = (
                            ...
                            f"gmx mdrun -v -deffnm {sim_name} -ntomp 12 -nb gpu -pme gpu -bonded gpu -pin on"
                            + f" &> ../run_{sim_name}.out"
                        )
                    # Otherwise, check log file for whether previous simulation finished correctly
                        ...
                        command = (
                            ...
                            + str(eq_extend)
                            + f" -o {sim_name}.tpr &&"
                            f"gmx mdrun -s {sim_name}.tpr -cpi {sim_name}.cpt -v -deffnm {sim_name} -ntomp 12 -nb gpu -pme gpu -bonded gpu -pin on"
                            + f" &> ../run_{sim_name}.out"
                        )
                    # Otherwise restart the simulation from the checkpoint file
                    else:
                        command = (
                            f"gmx mdrun -cpi {sim_name}.cpt -v -deffnm {sim_name} -ntomp 12 -nb gpu -pme gpu -bonded gpu -pin on"
                            + f" &> ../run_{sim_name}.out"
                        )
                    subprocess.run(command, shell=True, check=True, cwd=sim_name)
            ...
            # If the simulation fails because of insufficient convergence, extend the simulation
            if eq_ext_str in job.doc and job.doc[eq_ext_str] == True:
                job.doc[max_steps_str] = int(total_eq_steps + eq_extend * 40)
...
def __generate_DEC_xml(job):
 ...
 <NonbondedForce coulomb14scale="0.8333" lj14scale="0.5">
  ...
  <Atom type="C3" charge="1.055585" sigma="{sigma_C3:0.6f}" epsilon="{epsilon_C3:0.6f}"/>
