...
def create_system(job):
    ...
    # For BA implementation, set box size apriori
    box = [5.1, 5.1, 30.0]
...
@Project.post(lambda job: "liq_density_Pvap" in job.doc and "liq_density_unc_Pvap" in job.doc)
@Project.operation(cmd=False, directives={"omp_num_threads": 8})
def npzzat_prod_dens_calc(job):
    import panedr
    sys.path.append("../../")
    ...
    with job:  
        ...
        job.doc["liq_density_Pvap"] = dens_eq
        job.doc["liq_density_unc_Pvap"] = std

#Get density from NPT simulations
@LD_group
@IFT_group    
@Project.pre.after(npzzat_eq_sim)
@Project.post(lambda job: "ave_len_z" in job.doc and "ave_density" in job.doc)
@Project.operation(cmd=False, directives={"omp_num_threads": 8})
def npzzat_dens_calc(job):
    ...
    #Calculate the liquid density from the NPT eq run at high pressure
    with job:  
        density, volume, z_len = find_eq_time(job, last_sim_name)
...
def calculate_props(job):
    ...
    n_bulk_ops = [3,4,5,6,7,8,9,10,12,15,20,25]
        ...
        if prop == "Density":
            ...
            with job:
                if not os.path.exists(job.fn(f"{sim_name}/{name}.txt")):
                    for i in n_bulk_ops:
                        command = f"echo '{prop}' | gmx energy -f {last_dir}{last_sim_name}.edr -nbmin {i} -nbmax {i} -b 40000 >> {name}.txt"
                        subprocess.run(
                            command, text=True, check=True, shell=True, cwd = sim_name
                        )
            (means_est, vars_est, vars_err) = calc_blk_avg_energ(job, sim_name, name, n_bulk_ops)
...
#BA Helper functions
def find_eq_time(job, sim_name):
    """Find the time at which the system is equilibrated"""
    
    import panedr
    import numpy as np
    # This function is a placeholder and should be implemented based on the specific criteria for equilibration
    ...
    from_file = job.fn(f"{sim_name}/{sim_name}" + ".edr")
    # Get the density values from the NPT equilibration run
    df = panedr.edr_to_df(from_file)

    for prop in ["Density", "Volume"]:
        if prop == "Density":
            run_ave_Output_data = running_average(df[["Time", "Density"]])
            ...
            job.doc["final_equib_time"] = final_equib_time_stored

            values = np.array(df["Density"].values)
            density_equib_data = values[final_equib_time:]
            ave_density = np.mean(density_equib_data)
            job.doc["ave_density"] = ave_density
        else:
            values = np.array(df["Volume"].values)
            final_equib_time_stored = job.doc["final_equib_time"]
            vol_equib_data = values[final_equib_time:]
            ave_vol = np.mean(vol_equib_data)
            job.doc["ave_volume"] = ave_vol
            job.doc["ave_len_z"] = round(ave_vol/(5.1**2),5)

    return job.doc["ave_volume"], job.doc["ave_density"], job.doc["ave_len_z"]

# Calculation Functions
def calc_blk_avg_energ(job, sim_name, name, n_bulk_ops):
    ...
    with open(job.fn(f"{sim_name}/{name}_blk_avg.txt"), "w") as ferr:
        ...
        for nblk_ops, (mean_est, var_est, var_err) in enumerate(
            ...
            ferr.write(
                "{}\t{}\t{}\t{}\n".format(nblocks[nblk_ops], mean_est, var_est, var_err)
            )
...
def calc_block_densities(job, sim_name, last_sim_name, name):
    ...
    last_dir = f"../{last_sim_name}/"
    time_prod_strt = 40000

    with job:
        df_all_data = panedr.edr_to_df(job.fn(f"{last_sim_name}/{last_sim_name}.edr"))
        df_all = df_all_data[df_all_data["Time"] >= time_prod_strt]  # Filter out the equilibration time
        len_data = len(df_all)
...
nstlist                  = 100       ; 100 fs, largely irrelevant with Verlet
...
; Nonbonded settings 
cutoff-scheme            = Verlet    ; Buffered neighbor searching
verlet-buffer-tolerance  = 5e-3
ns_type                  = grid      ; search neighboring grid cells
nstlist                  = 50        ; 100 fs, largely irrelevant with Verlet
vdwtype                  = Cut-off   ; account for cut-off vdW scheme
rvdw                     = 1.2	     ;short-range van der Waals cutoff (in nm)
DispCorr                 = EnerPres        ; account for cut-off vdW scheme
...
ref_p                   = 100 100                   ; reference pressure, in bar
nstpcouple              = 5 
...
def _generate_inter_prod_mdp(job, cutoff):
    # Use 70000000 (50 ns) for the interfacial production
    contents = """
; MDP file for NVT simulation
...
""".format(
        temp=job.sp.T, nsteps=job.sp.nsteps_interprod
    )
